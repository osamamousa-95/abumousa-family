...
export default async function AdminHome({
  ...
  const [people, males, females, generations, admins, recent] = await Promise.all([
    prisma.person.count(),
  ...
  const cards = [
    ...
    { label: 'الأجيال', value: generations._max.generation ?? 0 },
          ...
          <ul className="mt-3 divide-y divide-[var(--color-line)] rounded-xl border border-[var(--color-line)]">
            {recent.map((r) => (
              <li key={r.id} className="flex flex-wrap items-center gap-2 px-4 py-2.5 text-sm">
                ...
                <span>{(r.after as { name?: string })?.name ?? (r.before as { name?: string })?.name ?? r.entity}</span>
