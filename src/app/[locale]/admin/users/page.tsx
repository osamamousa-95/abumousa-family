    ...
    <>
      ...
      <section className="mt-6 rounded-2xl border border-[var(--color-line)] bg-[var(--color-paper-2)] p-5">
        ...
        <p className="mt-1 text-xs text-[var(--color-muted)]">
          تُعطى كلمة مرور مؤقتة، ويُطلب من المشرف تغييرها عند أول دخول.
        </p>
        <div className="mt-4">
          <ActionForm action={createAdminUser} submitLabel="إضافة مشرف" resetOnSuccess>
      ...
      <ul className="mt-8 divide-y divide-[var(--color-line)] rounded-xl border border-[var(--color-line)]">
        {users.map((u) => (
          <li key={u.id} className="flex flex-wrap items-center gap-2 px-4 py-3 text-sm">
            ...
            {!u.isActive && (
              <span className="rounded-full bg-[var(--color-ox-100)] px-2.5 py-0.5 text-[11px] text-[var(--color-ox-700)]">معطَّل</span>
            )}
            {u.mustChangePassword && (
              <span className="rounded-full bg-[var(--color-gold-100)] px-2.5 py-0.5 text-[11px] text-[var(--color-gold-700)]">كلمة مرور مؤقتة</span>
            )}
            {u.id !== meId && (
              <span className="flex gap-1.5">
                <ConfirmButton
                  label={u.isActive ? 'تعطيل' : 'تفعيل'}
                  confirmText={u.isActive ? `تعطيل حساب ${u.username}؟` : `تفعيل حساب ${u.username}؟`}
                  onConfirm={() => setUserActive(u.id, !u.isActive)}
                />
                <ConfirmButton
                  label="حذف" danger
                  confirmText={`حذف حساب ${u.username} نهائياً؟`}
